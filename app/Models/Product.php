...
class Product extends Model
{
    ...
    // Status gabungan buat badge & rekap "Barang Habis":
    // - Produk yang dipantau kamera (ada yolo_label) -> pakai status_kamera, karena itu real-time
    //   dan tidak nunggu owner catat manual.
    // - Produk manual-saja (tanpa yolo_label) -> tetap pakai stok_pajangan, satu-satunya data yang ada.
    public function isHabisGabungan(): bool
    {
        if ($this->yolo_label) {
            return $this->status_kamera === 'habis';
        }
    ...
    public function scopeBarangHabisGabungan($query)
    {
        return $query->where(function ($q) {
            $q->whereNotNull('yolo_label')->where('status_kamera', 'habis');
        })->orWhere(function ($q) {
            $q->whereNull('yolo_label')->where('stok_pajangan', 0);
        });
    }
