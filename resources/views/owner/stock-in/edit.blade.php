...
@section('page-subtitle', 'Perbarui catatan penambahan stok')
...
<div class="space-y-4">
    ...
    <div class="bg-gradient-to-r from-blue-600 to-blue-700 rounded-xl p-4 md:p-6 text-white shadow-lg">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-lg md:text-2xl font-bold">✏️ Edit Stok Masuk</h2>
                <p class="text-blue-100 text-sm mt-0.5">
                    Dicatat pada {{ $stockIn->created_at->format('d M Y, H:i') }}
                </p>
            </div>
        </div>
    </div>
    ...
    {{-- FORM --}}
    <form method="POST" action="{{ route('owner.stock-in.update', $stockIn) }}" class="space-y-4">
        @csrf
        @method('PUT')
            ...
            <div class="p-5 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="number" name="jumlah"
                               value="{{ old('jumlah', $stockIn->jumlah) }}"
                               min="1" required
                    ...
                    <div>
                        ...
                        <select name="sumber" required
                            ...
                            @foreach ($sumber as $s)
                                <option value="{{ $s }}"
                                    {{ (old('sumber', $stockIn->sumber) === $s) ? 'selected' : '' }}>
                                    {{ ucfirst($s) }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <textarea name="keterangan" rows="3"
                              ...
                              class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition text-sm resize-none">{{ old('keterangan', $stockIn->keterangan) }}</textarea>
                </div>
