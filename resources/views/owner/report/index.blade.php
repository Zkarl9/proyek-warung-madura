...
@section('page-subtitle', 'Rekap pemasukan dan pengeluaran stok per periode')
@section('content')
...
<span>📊</span> Laporan Pemasukan & Pengeluaran
</h2>
...
<p id="errorTanggal" class="hidden text-xs font-bold text-rose-600 animate-pulse bg-rose-50 border border-rose-100 p-2.5 rounded-xl">⚠ Tanggal "Dari" tidak boleh lebih besar dari tanggal "Sampai".</p>
</div>
{{-- ══════════════════════════════════════════════════ --}}
...
<div class="min-w-0">
<p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Total Barang Masuk</p>
...
<div class="min-w-0">
<p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Total Barang Keluar</p>
...
{{-- Kartu keuangan: Pendapatan & Pengeluaran --}}
<div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
...
<div class="flex-shrink-0 w-12 h-12 rounded-xl bg-blue-50 border border-blue-100 text-blue-600 flex items-center justify-center text-xl shadow-inner">💰</div>
<div class="min-w-0">
<p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Total Pendapatan</p>
<p class="text-xl font-black text-blue-600 font-mono tracking-tight mt-0.5">Rp{{ number_format($omzet, 0, ',', '.') }}</p>
<p class="text-[10px] font-bold text-slate-400 mt-0.5 font-mono">Dari barang yang terjual</p>
</div>
</div>
<div class="bg-white rounded-2xl shadow-md border-2 border-slate-200/80 p-5 flex items-center gap-4 transition duration-200 hover:border-orange-400">
<div class="flex-shrink-0 w-12 h-12 rounded-xl bg-orange-50 border border-orange-100 text-orange-600 flex items-center justify-center text-xl shadow-inner">🧾</div>
<div class="min-w-0">
<p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Total Pengeluaran</p>
<p class="text-xl font-black text-orange-600 font-mono tracking-tight mt-0.5">Rp{{ number_format($hpp, 0, ',', '.') }}</p>
<p class="text-[10px] font-bold text-slate-400 mt-0.5 font-mono">Estimasi modal barang terjual</p>
</div>
</div>
</div>
@if ($adaProdukTanpaHargaBeli)
<div class="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl p-4 text-amber-800">
<span class="text-lg flex-shrink-0">⚠️</span>
<p class="text-xs font-medium leading-relaxed">
Sebagian produk yang terjual di periode ini belum pernah diisi <strong>harga beli</strong> di catatan pemasukan stoknya, jadi tidak ikut dihitung di Total Pengeluaran di atas (angkanya kemungkinan lebih rendah dari nilai sebenarnya). Isi harga beli tiap kali restock supaya perhitungannya makin akurat.
</p>
</div>
@endif
{{-- ══════════════════════════════════════════════════ --}}
...
<h3 class="text-xs font-bold text-slate-700 uppercase tracking-wider">Grafik Pemasukan & Pengeluaran Harian</h3>
</div>
<div class="p-5">
...
<div class="h-2 rounded-full bg-slate-100 overflow-hidden p-0.5 ring-1 ring-slate-200/50 mb-1">
...
<span class="text-slate-400">Omzet: <span class="text-slate-600">Rp{{ number_format($item['omzet'], 0, ',', '.') }}</span></span>
@if ($item['laba'] !== null)
<span class="{{ $item['laba'] >= 0 ? 'text-emerald-600' : 'text-rose-600' }}">Laba: Rp{{ number_format($item['laba'], 0, ',', '.') }}</span>
@else
<span class="text-slate-300 italic">Laba: belum diketahui</span>
@endif
</div>
</div>
@endforeach
...
<h3 class="text-xs font-bold text-slate-700 uppercase tracking-wider">Rincian Pemasukan</h3>
...
<p class="text-xs font-bold text-slate-500">Belum ada data pemasukan di rentang ini.</p>
</div>
@endforelse
</div>
</div>
...
<h3 class="text-xs font-bold text-slate-700 uppercase tracking-wider">Rincian Pengeluaran</h3>
...
<p class="text-xs font-bold text-slate-500">Belum ada data pengeluaran di rentang ini.</p>
</div>
@endforelse
</div>
</div>
...
scales: {
y: { beginAtZero: true, grid: { color: '#f1f5f9' }, ticks: { font: { size: 10, weight: '600' } } },
